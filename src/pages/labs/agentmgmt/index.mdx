...
## Prerequisites
  ...
  This tutorial assume that you have already started the Trial environment. If you haven't done it yet, please complete first the [Getting Started tutorial](../../gettingstarted/index.mdx)
...
## Create an API key
  ...
  ```sh
  cloudctl login -a $URL -u $USERBAME -p $PASSWORD -n $USERNAME --skip-ssl-validation
  ```
  ...
  export HZN_EXHANCGE_USER_AUTH=`cat $USERNAME.json | jq -r .apikey`
...
## Register the edge device with management hub
  ...
  ```sh
  hzn register -u iamapikey:$HZN_EXHANCGE_USER_AUTH
  ```
...
## Summary
  ...
  Congratulations! You have successfully completed the lab "Installing IBM Edge Application Manager agent”. Follow the [Service management tutorial](../svcmgmt/index.md) to learn more.
